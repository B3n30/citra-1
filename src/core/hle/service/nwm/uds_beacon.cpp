...
std::vector<u8> GenerateNintendoDummyTag() {
    // Note: These values were taken from a packet capture of an o3DS XL
    // broadcasting a Super Smash Bros. 4 lobby.
    static const std::vector<u8> dummy_data = { 0x0A, 0x00, 0x00 };
    ...
    tag.oui = NintendoOUI;
    std::memcpy(tag.data, dummy_data.data(), dummy_data.size());
...
std::vector<u8> GenerateNintendoNetworkInfoTag(const NetworkInfo& network_info) {
    NetworkInfoTag tag{};
    tag.header.tag_id = static_cast<u8>(TagId::VendorSpecific);
    tag.header.length = sizeof(NetworkInfoTag) - sizeof(TagHeader) + network_info.application_data_size;
    ...
    std::memset(tag.sha_hash, 0, sizeof(tag.sha_hash));
    ...
    std::memcpy(tag.network_info, &network_info.oui_value, sizeof(tag.network_info));
    ...
    std::memcpy(buffer.data() + sizeof(tag), network_info.application_data.data(), network_info.application_data_size);

    // Calculate the SHA1 of the contents of the tag.
    std::array<u8, CryptoPP::SHA1::DIGESTSIZE> hash;
    CryptoPP::SHA1().CalculateDigest(hash.data(), buffer.data() + offsetof(NetworkInfoTag, network_info),
...
std::vector<u8> GeneratedEncryptedData(const NetworkInfo& network_info, const NodeList& nodes) {
    ...
    for (const NodeInfo& node : nodes) {
        ...
        buffer.insert(buffer.end(), (u8*)&info, (u8*)&info + sizeof(info));
    }
 ...
 */
std::vector<u8> GenerateNintendoFirstEncryptedDataTag(const NetworkInfo& network_info, const NodeList& nodes) {
    const size_t payload_size = std::min<size_t>(EncryptedDataSizeCutoff,
 ...
 * The encrypted payload contains information about the nodes currently connected to the network.
 * This tag is only present if the payload size is greater than EncryptedDataSizeCutoff (0xFA) bytes.
 * @returns A buffer with the second Nintendo encrypted data parameters of the beacon frame.
 */
std::vector<u8> GenerateNintendoSecondEncryptedDataTag(const NetworkInfo& network_info, const NodeList& nodes) {
    ...
    std::vector<u8> encrypted_data = GeneratedEncryptedData(network_info, nodes);
    std::memcpy(buffer.data() + sizeof(tag), encrypted_data.data() + EncryptedDataSizeCutoff, payload_size);
 ...
 * @returns A buffer with the Nintendo tagged parameters of the beacon frame.
 */
std::vector<u8> GenerateNintendoTaggedParameters(const NetworkInfo& network_info, const NodeList& nodes) {
