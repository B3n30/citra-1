...
static void RecvBeaconBroadcastData(Interface* self) {
    ...
    std::deque<WifiPacket> beacons = room_member->PopWifiPackets(WifiPacket::PacketType::Beacon, mac_address);
    ...
    LOG_DEBUG(Service_NWM, "called out_buffer_size=0x%08X, wlan_comm_id=0x%08X, id=0x%08X,"
              "input_handle=0x%08X, out_buffer_ptr=0x%08X", out_buffer_size, wlan_comm_id,
              id, input_handle, out_buffer_ptr);
}
...
static void Bind(Interface* self) {
    ...
    LOG_DEBUG(Service_NWM, "called BindNodeID %d, data_channel %d, network_node_id %d",bind_node_id, data_channel, network_node_id);
...
static void SetApplicationData(Interface* self) {
    ...
    LOG_DEBUG(Service_NWM, "AppDataSize: %hhu", network_info.application_data_size);
...
static void ConnectToNetwork(Interface* self) {
    ...
    // TODO(B3N30): Actually implement this
...
static void DecryptBeaconData(Interface* self) {
    ...
    // TODO(Subv): Make this use IPC::RequestParser when possible.
    u32* cmd_buff = Kernel::GetCommandBuffer();
    const size_t output_buffer_size = cmd_buff[0x100 >> 2] >> 14;
    const VAddr output_buffer_addr = cmd_buff[(0x100 >> 2) + 1];
    ...
    // The first 4 bytes are the OUI and the OUI Type of the tags, we can ignore those.
    ...
    BeaconData data;
    std::memcpy(&data, beacon_data.data(), sizeof(data));
    ...
    for (int i = 0; i < num_nodes; ++i) {
        BeaconNodeInfo info;
        std::memcpy(&info, beacon_data.data() + sizeof(data) + i * sizeof(info), sizeof(info));
...
static void BeaconBroadcastCallback(u64 userdata, int cycles_late) {
    ...
    RoomMember* room_member = NetCore::getRoomMember();
    WifiPacket packet;
    packet.data = GenerateBeaconFrame(network_info, node_info);
    packet.channel = network_info.channel;
    packet.destination_address = NoPreferredMac;
